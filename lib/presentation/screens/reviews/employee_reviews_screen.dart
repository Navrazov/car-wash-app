...
class EmployeeReviewsScreen extends StatefulWidget {
  ...
  const EmployeeReviewsScreen({super.key, required this.employee});
...
class _EmployeeReviewsScreenState extends State<EmployeeReviewsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Отзывы о сотруднике'),
      ),
      body: _isLoading
        ...
        children: [
          Row(
            children: [
              ...
              if (review.employeeRating != null)
                _buildStars(review.employeeRating!),
            ],
          ),
          if (review.employeeComment != null && review.employeeComment!.isNotEmpty) ...[
            const SizedBox(height: 8),
            Text(
